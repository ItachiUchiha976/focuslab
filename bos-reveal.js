/* ============================================================
   BOS reveal au scroll — FocusLab — 16/07/2026
   ...
   classes reelles de FocusLab (product-card / feature-card /
   stat-card / section-tag, bouton #add-to-cart-btn).
   ============================================================ */
(function () {
  try {
    ...
    var selectors = [
      ...
      '.section-title', '.section-tag', '.section-sub',
      '#garanties .container > div > div'
    ];
    ...
    ['.products-grid', '.features-grid', '.stat-grid'].forEach(function (gSel) {
    ...
    /* ---- Barre d'achat sticky mobile (fiches produit) ----
       Construite depuis le bouton #add-to-cart-btn de la page :
       zero duplication de logique (le clic proxifie le vrai bouton,
       donc panier + Stripe + compteurs restent la source unique).
       FocusLab n'utilise pas [data-add-cart] : le CTA panier est
       identifie par son id et ses data-product-*. */
    (function stickyAtc() {
      var mainBtn = document.getElementById('add-to-cart-btn');
      if (!mainBtn || document.querySelector('.bos-sticky-atc')) return;
      var name = mainBtn.getAttribute('data-product-name') || document.title.split(String.fromCharCode(8212))[0];
      var price = mainBtn.getAttribute('data-product-price');
